...
public class TrainLine {
	...
	public int getSize() {
		...
		/*
		 * //If the line is empty we return 0 if(leftTerminus==null &&
		 * rightTerminus==null) { return 0; }
		 */
	...
	public TrainStation travelOneStation(TrainStation current, TrainStation previous) {
		...
		/*
		 * transfer to it if it was our previous then transfer to next station
		 */
	...
	public TrainStation findStation(String name) {
		 ...
		 * TrainStation tempStation = leftTerminus;
		 * 
		 * while(tempStation.getRight()!=null) { if(tempStation.getName().equals(name))
		 * { return tempStation; }else { tempStation=tempStation.getRight(); } }
		 */
		...
		while (repeat) {
			...
			for (int i = 0; i < lineMap.length - 1 - lastIndex; i++) {
				...
				if (lineMap[i].getName().compareTo(lineMap[i + 1].getName()) > 0) {
					...
					// System.out.println("we switched " + lineMap[i+1].getName() + " with " +
					// lineMap[i].getName() );
					repeat = true;
				}
	...
	public TrainStation[] getLineArray() {
		...
		/*
		 * //if there are no elements return null if (leftTerminus==null &&
		 * rightTerminus==null) { return null; }
		 */

		// might not be good because the reference might not exist after the method is
		// done
